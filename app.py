...
# =========================
# OS-tips – Vinter-OS 2026 (Sverige)
# =========================
...
def load_athletes() -> pd.DataFrame:
    """
    Förväntar data/athletes.csv med kolumner:
      athlete_id,name,sport,event
    event kan vara tomt, men i denna uppläggning är varje gren en egen rad.
    """
    if not ATHLETES_CSV.exists():
        st.error("Saknar data/athletes.csv i repot.")
        st.stop()
    ...
    df["display"] = df["name"]
    df.loc[df["event"].str.strip() != "", "display"] = df["name"] + " — " + df["event"]

    return df.sort_values(["sport", "event", "name"]).reset_index(drop=True)


def load_results(athletes: pd.DataFrame) -> pd.DataFrame:
    ensure_state_dir()

    # Om results.csv saknas i state-katalogen: skapa default (None)
    ...
    # Säkerställ att alla atleter finns med
...
def score_pick(pick: str, actual: str) -> int:
    # 5p exakt medaljvalör + person/rad, 2p rätt person/rad men fel valör
    if actual == "None":
        return 0
    if pick == actual:
        return 5
...
st.caption("Varje gren är en egen rad i athletes.csv. Tips och resultat kan exporteras via Backup/Restore.")
...
with tabs[0]:
    ...
    with left:
        ...
        st.subheader("Välj sport")
        sport = st.selectbox("Sport", sorted(athletes["sport"].unique().tolist()))
        subset_sport = athletes[athletes["sport"] == sport].copy()

        has_events = subset_sport["event"].str.strip().ne("").any()
        if has_events:
            st.subheader("Välj gren")
            events = sorted(subset_sport["event"].str.strip().unique().tolist())
            event = st.selectbox("Gren", events)
            subset = subset_sport[subset_sport["event"].str.strip() == event].copy()
        else:
            subset = subset_sport

        st.subheader("Välj atlet")
        athlete_label_map = {row["display"]: row["athlete_id"] for _, row in subset.iterrows()}
        ...
        with colA:
            if st.button("Spara/uppdatera tips", use_container_width=True):
                ...
                st.success("Sparat!")
        with colB:
            if st.button("Ta bort tips för denna rad", use_container_width=True):
    ...
    with right:
        ...
        if not user_picks:
            ...
            picked_df = pd.DataFrame([{"athlete_id": k, "pick": v} for k, v in user_picks.items()])
            view = athletes.merge(picked_df, on="athlete_id", how="right")
            view = view[["sport", "event", "name", "athlete_id", "pick"]].sort_values(["sport", "event", "name"]).reset_index(drop=True)
...
with tabs[1]:
    ...
    st.subheader("Resultat (admin fyller i)")
    res_view = results.merge(athletes[["athlete_id", "name", "sport", "event"]], on="athlete_id", how="left")
    res_view = res_view[["sport", "event", "name", "athlete_id", "medal"]].sort_values(["sport", "event", "name"]).reset_index(drop=True)
    st.dataframe(res_view, use_container_width=True, hide_index=True)

    st.caption("Poäng: 5p exakt medaljvalör, 2p om rätt person/rad tog medalj men fel valör.")
...
with tabs[2]:
    ...
    if not ok:
        ...
        res = results.merge(athletes[["athlete_id", "name", "sport", "event"]], on="athlete_id", how="left")
        res = res[["sport", "event", "name", "athlete_id", "medal"]].sort_values(["sport", "event", "name"]).reset_index(drop=True)

        updated = []
        for _, row in res.iterrows():
            c1, c2, c3, c4, c5 = st.columns([2.0, 2.5, 2.5, 2.5, 1.5])
            with c1: st.write(row["sport"])
            with c2: st.write(row["event"] if str(row["event"]).strip() else "—")
            with c3: st.write(row["name"])
            with c4: st.write(row["athlete_id"])
            with c5:
